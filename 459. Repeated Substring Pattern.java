class Solution {
    public boolean repeatedSubstringPattern(String s) {
        ...
        //return solveUsingNaiveApproach(s, len);
        
    }

    // Approach 1: O(n * root of n)
    boolean solveUsingNaiveApproach(String s, int len){
        for(int subLen = 1; subLen <= len/2; subLen++){
            String pattern = s.substring(0, subLen);
            boolean found = isRepeatedPattern(s, pattern, len, subLen);
            if(found) return true;
        }
        return false;
    }

    boolean isRepeatedPattern(String s, String pattern, int len, int subLen){
        if(len % subLen != 0) return false;
        int parts = len /subLen;
